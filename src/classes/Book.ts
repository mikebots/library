...
export default class Book implements BookInterface {
  ...
  constructor(settings: BookSettings) {
    this._id = settings.id;
    this._path = settings.path;

    this.settings_path = path.join(this._path, "settings.json");

    this.model_name = settings.model_name;
    this.settings = settings.settings as BookPathSettings;
    this.init();
  }
  init() {
    if (!Util.exists(this._path) || !Util.exists(this.settings_path)) {
      ...
      Util.writeFile(
        this.settings_path,
        JSON.stringify({ ...Util.defaultBookSettings(this), ...this.settings })
      );
    } else {
  ...
  get _settings(): BookPathSettings {
  ...
  get _pages() {
    ...
    let files = Array.from(
      Util.listFiles(this._path).filter((file) => file !== "settings.json")
    );
    if (files.length === 0) return pages;
    for (const file of files) {
      let pageFileName = file.split(".")[0];
      let pageSettings = this._settings.pages.find(
        (page: PageSettings) => page.id === pageFileName
      ) as PageSettings;
      let page = new Page({
        id: pageFileName,
  ...
  checkIfFull(data?: any, page_id?: string) {
    if (this._full) throw new Error("Book is full");
    if (data) {
      if (page_id) {
        let pageSettings = this.getPageSettings(page_id);
        if (!pageSettings) throw new Error("Page not found");
        let page = this.getPage(page_id);
        if (!page) throw new Error("Page not found");
        if (page._full) throw new Error("Page is full");
        if (
          Util.getObjectSizeInBytes(data) + page._s > pageSettings.maxSize &&
  ...
  getPageSettings(page_id: string) {
    return this._settings.pages.find((page) => page.id === page_id);
  }
  create_page() {
    this.checkIfFull();
    let id = Util.generateId(
      this._pages.map((page) => page._id),
      new Date(Date.now())
    );
    let page_path = path.join(this._path, id + ".json");
    let page = new Page({
      id,
      path: page_path,
  ...
  delete_page(page_id: string) {
    let page = this.getPage(page_id);
    if (page) {
      // step 1, remove from settings
      let pages = this._settings.pages.filter(
        (page: PageSettings) => page.id !== page_id
      );
      this.updateSettings({ pages });
      // step 2, remove from file system
      Util.deleteFile(page._path);

      return true;
    }
    return false;
  }
  edit_page(page_id: string, data: any) {
    let page = this.getPage(page_id);
    if (page) {
  ...
  get rules() {
    return new Rules(this._settings.rules);
  }
  getPage(page_id: string) {
    return this._pages.find((page) => page._id === page_id);
  }
  updatePageSetting(page_id: string, settings: any) {
    let pages = this._settings.pages;
  ...
  updatePageInSettings(page_id: string) {
    let pages = this._settings.pages;
    let page = pages.find((page: PageSettings) => page.id === page_id);
    if (page) {
  ...
  save_data(data: any) {
    ...
    let page =
      this._pages.find((p) => {
        if (p._maxSize == 0) {
          if (this._settings.pageMaxSize == 0) return true;
          else
            return (
              Util.getObjectSizeInBytes(data) + p._s <=
              this._settings.pageMaxSize
            );
        } else return Util.getObjectSizeInBytes(data) + p._s <= p._maxSize;
      }) || this.create_page();

    let validation = this.rules.validate(data, true);
    if (!validation.success) throw new Error(validation.message);
    let id = Util.generateId(this.getModelIDS(), new Date(Date.now()));
  ...
  findAll(query: any) {
    ...
    for (const page of pages) {
      ...
      for (const foundModel of found) {
        let result = new Result(foundModel, page._id, this._id);
        models.push(result);
      }
    }
    return models;
  ...
  findOne(query: any) {
    ...
    for (const page of pages) {
      let found = page.getModels(query);
      if (found.length > 0) return new Result(found[0], page._id, this._id);
    }
    return null;
  }
  getModel(id: string) {
  ...
  setPageMaxSize(page_id: string, maxSize: number) {
    let pages = this._settings.pages;
    let page = pages.find((page: PageSettings) => page.id === page_id);
    if (page) {
  ...
  setAllPagesMaxSize(maxSize: number) {
  ...
  audit() {
    ...
    let files = Util.listFiles(this._path).filter(
    ...
    for (const file of files) {
      ...
      if (
        !this._settings.pages.find(
          (pageSetting) => pageSetting.id == nameWithoutJSON
        )
      )
        throw new Error(
          `Page (${nameWithoutJSON}) is not in the settings of book (${this._id}). Please manually fix this error.`
        );
    }
    let size = this._s;
    ...
    if (maxSize != 0 && size > maxSize)
      throw new Error(
        `Book (${this._id}) is full and exceeds max size by ${
          size - maxSize
        }. Please manually fix this error.`
      );
    //step 2.2
    let pages = this._pages;
    ...
    for (const pageSetting of pageSettings) {
      if (!pages.find((page) => page._id == pageSetting.id))
        throw new Error(
          `Page (${pageSetting.id}) is missing from book (${this._id}). Please manually fix this error.`
        );
    }
    ...
    for (const page of pages) {
      if (!pageSettings.find((pageSetting) => pageSetting.id == page._id))
        throw new Error(
          `Page (${page._id}) is not in the settings of book (${this._id}). Please manually fix this error.`
        );
    }
    for (const page of pages) {
      let size = page._s;
      ...
      if (maxSize != 0 && size > maxSize)
        throw new Error(
          `Page (${page._id}) is full and exceeds max size by ${
            size - maxSize
          }. Please manually fix this error.`
        );
    }
    ...
    for (const page of pages) {
      ...
      for (const model of page._models) {
        ...
        if (!validation.success)
          throw new Error(
            `Model (${model._id}) in page (${page._id}) of book (${this._id}) is invalid. Please manually fix this error.\nError Message: ${validation.message}`
          );
        ...
        for (const prop of required) {
          if (!model.hasOwnProperty(prop))
            throw new Error(
              `Model (${model._id}) in page (${page._id}) of book (${this._id}) is missing property (${prop}). Please manually fix this error.`
            );
        }
        ...
        if (doc.toISOString() == "Invalid Date")
          throw new Error(
            `Model (${model._id}) in page (${page._id}) of book (${this._id}) has an invalid date of creation (${model._doc}). Please manually fix this error.`
          );
        // get difference in milliseconds
        let diff = lastModified.getTime() - doc.getTime();
        if (diff < 0)
          throw new Error(
            `Model (${model._id}) in page (${page._id}) of book (${
              this._id
            }) has a date of creation (${doc.toISOString()}) that is further ahead than the last modification date (${lastModified.toISOString()}). Please manually fix this error.`
          );
        //now check if the size is correct
        let size = model._s;
        let objectSize = Util.getObjectSizeInBytes(model);
        ...
        if (pid != page._id)
          throw new Error(
            `Model (${model._id}) in page (${page._id}) of book (${this._id}) has a page id (${pid}) that does not match the actual page id (${page._id}). Please manually fix this error.`
          );
        //now check if the name is correct
        let name = model._n;
        if (name != this.model_name)
          throw new Error(
            `Model (${model._id}) in page (${page._id}) of book (${this._id}) has a name (${name}) that does not match the actual name (${this.model_name}). Please manually fix this error.`
          );
      }
    }
    ...
    for (const page of pages) {
      let pageSetting = pageSettings.find(
        (pageSetting) => pageSetting.id == page._id
      );
      if (!pageSetting)
        throw new Error(
          `Page (${page._id}) is not in the settings of book (${this._id}). Please manually fix this error.`
      ...
      if (pageSetting.book_id != this._id)
        throw new Error(
          `Page (${page._id}) has a book id (${pageSetting.book_id}) that does not match the book id (${this._id}). Please manually fix this error.`
        );
      if (page._doc != pageSetting.doc)
        throw new Error(
          `Page (${page._id}) has a date of creation (${page._doc}) that does not match the date of creation (${pageSetting.doc}) in the settings. Please manually fix this error.`
        );
      if (page._lm != pageSetting.lm)
        throw new Error(
          `Page (${page._id}) has a last modification date (${page._lm}) that does not match the last modification date (${pageSetting.lm}) in the settings. Please manually fix this error.`
        );
      ...
      if (page._path != pageSetting.path)
        throw new Error(
          `Page (${page._id}) has a path (${page._path}) that does not match the path (${pageSetting.path}) in the settings. Please manually fix this error.`
        );
    }
    ...
    if (this._doc != this._settings.doc)
      throw new Error(
        `Book (${this._id}) has a date of creation (${this._doc}) that does not match the date of creation (${this._settings.doc}) in the settings. Please manually fix this error.`
      );
    if (this._id != Util.getFileOrFolderName(this._path))
      throw new Error(
        `Book (${this._id}) has a path (${
          this._path
        }) that does not match the id (${Util.getFileOrFolderName(
          this._path
        )}) in the settings. Please manually fix this error.`
      );
    if (this._path != this._settings.path)
      throw new Error(
        `Book (${this._id}) has a path (${this._path}) that does not match the path (${this._settings.path}) in the settings. Please manually fix this error.`
      );
    if (this.model_name != this._settings.model_name)
      throw new Error(
        `Book (${this._id}) has a model name (${this.model_name}) that does not match the model name (${this._settings.model_name}) in the settings. Please manually fix this error.`
      );

    return true;
  }
  getPageBySettings(pageSettings: PageSettings) {
    return new Page(pageSettings);
  }
}
